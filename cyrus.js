...
async function checkAutoRotates(env) {
	...
	try {
		...
		const res = await fetch("https://updates.cyruspanel.local/ips.txt");
		if (!res.ok) return;
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const m2 = decodeURIComponent("%F0%9F%9A%80%40CYRUS_PANEL%20%D8%B3%D8%A7%D8%AE%D8%AA%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%F0%9F%9A%80");
...
function setModalState(modalId, show) {
	...
	async function checkGlobalMessage() { return;
		...
		const m2 = decodeURIComponent('%E2%99%A8%EF%B8%8F%20%40CYRUS_PANEL%20%D8%B3%D8%A7%D8%AE%D8%AA%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%20%E2%99%A8%EF%B8%8F');
...
async function fetchIpsList() {
    try {
        const IP_REPO_URL = (window.CYRUS_IP_REPO_URL || 'https://raw.githubusercontent.com/USERNAME/REPO/main/ips.txt');
